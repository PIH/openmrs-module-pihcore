 ...
 */
public class CheckInsByAgeReportFragmentController {
    ...
    public SimpleObject evaluate(@SpringBean DataSetDefinitionService dataSetDefinitionService,
                           @SpringBean CheckInDataSetManager checkInDataSetManager,
     ...
     */
    protected AgeResult getAgeResult(Map<String, AgeResult> checkInsByAge, String ageCategory) {
        AgeResult ageResult = checkInsByAge.get(ageCategory);
        if (ageResult == null) {
            ageResult = new AgeResult();
            ageResult.setAgeCategory(ageCategory);
            checkInsByAge.put(ageCategory, ageResult);
        }
        return ageResult;
    }
