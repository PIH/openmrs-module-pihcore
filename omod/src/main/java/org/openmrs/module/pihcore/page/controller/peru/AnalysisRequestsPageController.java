...
// by uiframework convention, this get automatically wired in as the controller for the url "pihcore/peru/analysisRequests.page"
...
// TODO we could also rename this depending on how it ends up fitting into the whole "analysis requests" workflow
public class AnalysisRequestsPageController {
    ...
    public void get(PageModel model,   //  a model we can add data to, that will then passed to the view
        ...
        // (see EncounterService class in OpenMRS Core for full API:https://github.com/openmrs/openmrs-core/blob/2.3.x/api/src/main/java/org/openmrs/api/EncounterService.java)

        // add this list to the model under "encounters"
        model.put("encounters", encounters);

        // we don't need to specific view here... since the controller is named "AnalysisRequestsPageController" and it's in the peru package.
