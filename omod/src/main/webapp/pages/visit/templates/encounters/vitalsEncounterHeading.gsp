...
<div class="row col-12">
    <span class="encounter-card-summary">{{ encounter.obs | byConcept:Concepts.temperature:true | obs:{ appendComma:true } }} {{ encounter.obs | byConcept:Concepts.heartRate:true | obs:{ appendComma:true } }} {{ encounter.obs | byConcept:Concepts.respiratoryRate:true | obs }}</span>
</div>
<div class="row justify-content-between">
    <div class="col-8">
        <span class="encounter-card-summary provider-and-location-span">{{ 'emr.by' | translate }} {{ encounter.encounterProviders | encounterRole:primaryEncounterRoleUuid | getProviderName }}</span>
