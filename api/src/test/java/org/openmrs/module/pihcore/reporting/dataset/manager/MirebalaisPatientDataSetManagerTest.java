...
public class MirebalaisPatientDataSetManagerTest extends EncounterDataSetManagerTest {

    @Autowired
    MirebalaisPatientDataSetManager mirebalaisPatientDataSetManager;

    @Test
    public void testDataSet() throws Exception {
        Patient p = createPatient("X3XK71");
        Location biwoResepsyon = locationService.getLocation("Biwo Resepsyon");
        Visit visit = createVisit(p, biwoResepsyon);
        createRegistrationEncounter(p, visit);

        DataSetDefinition dsd = mirebalaisPatientDataSetManager.constructDataSet();
        ...
        Assert.assertEquals("X3XK71", row.getColumnValue("zlemr"));
    }
    ...
    public void testDataSetFiltersByVisitLocationWhenSpecified() throws Exception {
        ...
        context.addParameterValue("startDate", DateUtil.getDateTime(2015, 1, 1));
        context.addParameterValue("endDate", DateUtil.getDateTime(2015, 12, 31));
        context.addParameterValue("location", biwoResepsyon);

        SimpleDataSet dataSet = (SimpleDataSet) dataSetDefinitionService.evaluate(dsd, context);
        Assert.assertEquals(1, dataSet.getRows().size());
        Assert.assertEquals("X1AAAD", dataSet.getRows().get(0).getColumnValue("zlemr"));
    }

    @Test
    public void testDataSetReturnsAllWhenVisitLocationNotSpecified() throws Exception {
        Patient p1 = createPatient("X1AAAD");
        Patient p2 = createPatient("X2CCC6");
    ...
    @Test
    public void testDataSetExcludesPatientsWithRegistrationEncounterButNoVisit() throws Exception {
